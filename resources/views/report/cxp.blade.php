...
@extends('layouts.app')
@section('title', __('Cuentas por pagar - Reporte'))

@section('content')
    <section class="content-header">
        <h1>@lang('Cuentas por pagar - Reporte')</h1>
    </section>

    <section class="content">
        <div class="print_section">
            <h2>{{ session()->get('business.name') }} - @lang('Cuentas por pagar')</h2>
        </div>
...
{{--                     <div class="col-md-3">
            ...
            <div class="col-md-12">
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        <li class="active">
                            <a href="#cxp_by_month" data-toggle="tab" aria-expanded="true">
                                <i class="fa fa-calendar"></i> @lang('CXP por mes')
                            </a>
                        </li>
                    </ul>

                    <div class="tab-content">
                        <div class="tab-pane active" id="cxp_by_month">
                            <button id="generate_report_cxp" class="btn btn-primary mb-3 text-center">Generar
                                Reporte</button>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal fade" id="view_cxp_report_modal" tabindex="-1" role="dialog"
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            $(document).on('click', '#generate_report_cxp', function() {
                ...
                let url = '{{ route('cxp.generateReport') }}';
                let data = {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        $('#view_cxp_report_modal').html(result).modal('show');
                        __currency_convert_recursively($('#view_cxp_report_modal'));
                    },
                    error: function(xhr) {
        ...
        function printThisCxp() {
            $("#cxpModalReport").printThis({
                importCSS: true,
                importStyle: true,
